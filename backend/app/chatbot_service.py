...
def generate_fallback_analysis(query: str, location: str, weather_context: Optional[Dict[str, Any]], lang_code: str) -> str:
    """Intelligent Q&A Meteorological Analyzer in requested language."""
    ...
    if lang_code == "ta":
        ...
        return f"💡 **Direct Answer for: \"{query}\"**\n\n• **Location Context**: {loc} ({temp}°C, {condition})\n• **Analysis**: Here is the specific meteorological breakdown for your query: Conditions are evaluated based on real-time satellite telemetry and atmospheric data."
...
async def process_chatbot_query(
    ...
    if api_key:
        ...
        system_prompt = (
            ...
            "CRITICAL MANDATES:\n"
            f"1. DIRECT ANSWER: You MUST answer the user's SPECIFIC question directly. Do NOT dump raw weather metrics unless the user explicitly asks for current weather/temperature.\n"
            f"2. LANGUAGE: Write your ENTIRE response natively in {lang_name} (language code: '{lang_code}').\n"
            "3. RAIN / FORECAST QUESTIONS: If asked whether it will rain, start on Line 1 with an explicit bold YES or NO verdict (e.g. 'YES 🌧️ — Rain is expected...' or 'NO ☀️ — Rain is unlikely...').\n"
            "4. NO PLACEHOLDERS: Provide clear, educational, professional answers."
        )
