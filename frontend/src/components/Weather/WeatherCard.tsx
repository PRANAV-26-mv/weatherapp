...
export const WeatherCard: React.FC<WeatherCardProps> = ({
  ...
  return (
    <div className="glass-card p-4 sm:p-5 relative overflow-hidden group">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5 mb-3">
        <div>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <span className="text-[10px] px-1.5 py-0.5 rounded-md glass-pill text-gray-300 font-medium">
              {weather.country}
            </span>
          ...
          <p className="text-[10px] text-gray-400 mt-0.5 flex items-center gap-1">
            ...
            <span className="text-saffron font-semibold ml-1.5">Live Open-Meteo Sensor</span>
          </p>
        </div>

        <div className="flex items-center gap-2">
          <VoiceButton mode="output" textToSpeak={speechText} langCode={langCode} />
          <div className="px-2 py-0.5 rounded-full bg-saffron/10 border border-saffron/30 text-saffron text-[10px] font-semibold flex items-center gap-1">
      ...
      {/* Main Temp & Condition */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-center mb-3.5">
        <div className="flex items-baseline gap-2.5">
          <span className="text-4xl sm:text-5xl font-extrabold text-white tracking-tighter font-heading">
            {weather.tempC}°<span className="text-xl font-light text-saffron">C</span>
          </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 sm:gap-2.5">
        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          ...
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.humidity}%</span>
        </div>
        ...
        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          ...
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.windSpeedKmh} <span className="text-[9px] font-normal text-gray-400">km/h</span></span>
        </div>
        ...
        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          ...
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.windDirectionText}</span>
        </div>

        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          <Gauge className="w-3.5 h-3.5 text-purple-400 mb-0.5" />
          <span className="text-[10px] text-gray-400 font-medium">{translate('pressure', langCode)}</span>
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.pressureHpa} <span className="text-[9px] font-normal text-gray-400">hPa</span></span>
        </div>

        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          <Sun className="w-3.5 h-3.5 text-yellow-400 mb-0.5" />
          <span className="text-[10px] text-gray-400 font-medium">{translate('uv_index', langCode)}</span>
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.uvIndex}</span>
        </div>

        <div className="bg-white/5 p-2 sm:p-2.5 rounded-xl border border-white/5 flex flex-col items-center text-center">
          <Eye className="w-3.5 h-3.5 text-cyan-400 mb-0.5" />
          <span className="text-[10px] text-gray-400 font-medium">{translate('visibility', langCode)}</span>
          <span className="text-xs sm:text-sm font-bold text-white mt-0.5">{weather.visibilityKm} <span className="text-[9px] font-normal text-gray-400">km</span></span>
        </div>
      </div>
