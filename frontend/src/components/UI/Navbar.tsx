...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 glass-panel border-b border-slate-200 dark:border-white/10 px-3 lg:px-6 py-1.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 xl:gap-4">
        ...
        <div className="relative hidden md:flex items-center gap-1.5 w-44 lg:w-56 xl:w-64">
          <form onSubmit={handleSearchSubmit} className="relative flex-1">
            <input
              ...
              className="w-full glass-input text-[11px] text-slate-900 dark:text-white pl-7 pr-6 py-1 rounded-full border border-slate-300 dark:border-white/15 focus:border-saffron"
            />
            <Search className="w-3 h-3 text-slate-400 dark:text-gray-400 absolute left-2.5 top-2" />
          </form>
          ...
          {searchResults.length > 0 && (
            <div className="absolute top-9 left-0 right-0 glass-panel rounded-xl border border-slate-200 dark:border-white/15 shadow-2xl p-1.5 z-50">
              <div className="text-[9px] text-amber-800 dark:text-saffron uppercase font-bold px-2 py-0.5">Select Location</div>
              {searchResults.map((loc, idx) => (
                <button
                  ...
                  className="w-full text-left px-2.5 py-1 text-[11px] text-slate-800 dark:text-gray-200 hover:text-slate-900 dark:hover:text-white hover:bg-saffron/20 rounded-lg flex items-center justify-between"
                >
                  <span className="font-semibold">{loc.name}</span>
                  <span className="text-[9px] text-slate-500 dark:text-gray-400">{loc.country}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <nav className="hidden lg:flex items-center gap-1 shrink-0">
          ...
          <div className="relative">
            ...
            {moreDropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-52 glass-panel rounded-xl border border-slate-200 dark:border-white/15 shadow-2xl p-1.5 z-50">
                <div className="text-[9px] text-amber-800 dark:text-saffron uppercase font-bold px-2.5 py-0.5 border-b border-slate-200 dark:border-white/10 mb-1">
                  Specialized Portals
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-2.5 py-1.5 text-[11px] rounded-lg flex items-center gap-2 transition-all ${
                        isActive
                          ? 'bg-saffron text-black font-bold'
                          : 'text-slate-700 dark:text-gray-200 hover:bg-saffron/20 hover:text-slate-900 dark:hover:text-white'
                      }`}
                    >
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="p-1.5 rounded-full glass-pill text-slate-700 dark:text-gray-300 hover:text-amber-600 dark:hover:text-saffron hover:border-saffron/50 transition-all flex items-center justify-center"
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-1 px-2 py-1 rounded-full glass-pill text-[10px] font-bold text-slate-700 dark:text-gray-300 hover:text-amber-600 dark:hover:text-saffron transition-all"
          ...
          <button
            ...
            className={`px-2 py-1 rounded-full text-[10px] font-bold flex items-center gap-1 transition-all ${
              isLowDataMode
                ...
                : 'glass-pill text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white'
            }`}
          ...
          <button
            ...
            className="relative p-1.5 rounded-full glass-pill text-slate-700 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white hover:border-saffron/50 transition-all"
          ...
          <button
            ...
            className="lg:hidden p-1.5 rounded-lg glass-pill text-slate-700 dark:text-gray-200"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4 animate-fade-in">
          <div className="glass-panel p-6 rounded-3xl border border-saffron/40 max-w-md w-full space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-amber-800 dark:text-saffron font-bold text-base font-heading">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-1">
                <div className="flex items-center gap-2 font-bold text-amber-800 dark:text-saffron">
                  <Cpu className="w-4 h-4" />
                  <span>Core Generative LLM & NLU Engine:</span>
                </div>
                <div className="text-slate-800 dark:text-gray-200 pl-6">
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 space-y-1">
                ...
                <div className="text-slate-600 dark:text-gray-300 text-[11px] pl-6">
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden mt-3 pt-3 border-t border-slate-200 dark:border-white/10 space-y-3 text-xs max-h-[75vh] overflow-y-auto pr-1">
          ...
          <div className="relative">
            <form onSubmit={handleSearchSubmit} className="relative">
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full glass-input text-xs text-slate-900 dark:text-white pl-8 pr-4 py-2 rounded-xl border border-slate-300 dark:border-white/15 focus:border-saffron"
              />
            ...
            {searchResults.length > 0 && (
              <div className="mt-1 glass-panel rounded-xl border border-slate-200 dark:border-white/15 shadow-2xl p-1.5 z-50 space-y-1">
                <div className="text-[9px] text-amber-800 dark:text-saffron uppercase font-bold px-2 py-0.5">Select Location</div>
                {searchResults.map((loc, idx) => (
                  <button
                    key={idx}
                    ...
                    className="w-full text-left px-2.5 py-1.5 text-xs text-slate-800 dark:text-gray-200 hover:text-slate-900 dark:hover:text-white hover:bg-saffron/20 rounded-lg flex items-center justify-between"
                  >
                    <span className="font-semibold">{loc.name}</span>
                    <span className="text-[10px] text-slate-500 dark:text-gray-400">{loc.country}</span>
                  </button>
                ))}
              </div>
            )}
                ...
                <button
                  ...
                  className={`p-2.5 rounded-xl font-semibold flex items-center gap-2 transition-all ${
                    isActive
                      ...
                      : 'bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10'
                  }`}
                >
