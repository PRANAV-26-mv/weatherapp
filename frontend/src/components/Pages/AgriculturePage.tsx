    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="glass-card p-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-700 dark:text-gray-300 font-medium">Selected Crop:</span>
          <select
            value={selectedCrop}
            onChange={(e) => setSelectedCrop(e.target.value)}
            className="glass-input px-3 py-1.5 rounded-xl text-xs font-bold border border-slate-200 dark:border-indiagreen/40 bg-white dark:bg-black/60 text-slate-900 dark:text-white focus:border-indiagreen"
          >
            {cropOptions.map((c) => (
              <option key={c} value={c} className="bg-white text-slate-900 dark:bg-gray-900 dark:text-white">{c}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass-card p-5 space-y-2 border-t-4 border-t-blue-500">
          ...
          <div className="text-2xl font-bold text-slate-900 dark:text-white">{advisoryData.soilMoisturePct}% Moisture</div>
          <p className="text-xs text-slate-600 dark:text-gray-300 leading-relaxed">{advisoryData.irrigationRecommendation}</p>
        </div>
        ...
        <div className="glass-card p-5 space-y-2 border-t-4 border-t-amber-500">
          ...
          <div className="text-2xl font-bold text-slate-900 dark:text-white">Moderate Threat</div>
          <p className="text-xs text-slate-600 dark:text-gray-300 leading-relaxed">{advisoryData.pestDiseaseRisk}</p>
        </div>
        ...
        <div className="glass-card p-5 space-y-2 border-t-4 border-t-emerald-500">
          ...
          <div className="text-2xl font-bold text-slate-900 dark:text-white">3-Day Dry Window</div>
          <p className="text-xs text-slate-600 dark:text-gray-300 leading-relaxed">{advisoryData.weatherWindow}</p>
        </div>

        <div className="glass-card p-5 space-y-2 border-t-4 border-t-saffron">
          <div className="flex items-center gap-2 text-xs text-amber-700 dark:text-saffron font-bold">
            <Sun className="w-4 h-4" />
            <span>Heat Stress Index</span>
          </div>
          <div className="text-2xl font-bold text-slate-900 dark:text-white">{advisoryData.heatStressLevel} Heat Stress</div>
          <p className="text-xs text-slate-600 dark:text-gray-300 leading-relaxed">{advisoryData.harvestOutlook}</p>
        </div>
      </div>
      ...
      <div className="glass-card p-6 space-y-4">
        ...
        <div className="space-y-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-100/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-start gap-3">
            <CheckCircle2 className="w-4 h-4 text-indiagreen shrink-0 mt-0.5" />
            <div>
              <strong className="text-slate-900 dark:text-white block font-bold">Fertilizer Application:</strong>
              <span className="text-slate-600 dark:text-gray-300">Apply secondary top dressing of Urea (25 kg/acre) during morning calm hours when wind speed is &lt; 10 km/h.</span>
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-slate-100/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-start gap-3">
            <CheckCircle2 className="w-4 h-4 text-indiagreen shrink-0 mt-0.5" />
            <div>
              <strong className="text-slate-900 dark:text-white block font-bold">Drainage Management:</strong>
              <span className="text-slate-600 dark:text-gray-300">Ensure field drainage channels remain clear to prevent water stagnation if rainfall exceeds 25mm.</span>
            </div>
          </div>
        </div>
      </div>
